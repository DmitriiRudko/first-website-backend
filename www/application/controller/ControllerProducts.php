...
class ControllerProducts extends Controller {
    ...
    public function showProducts() {
        ...
        $amountOfProducts = $this->modelProducts->countProducts();
        if ($productIndex >= $amountOfProducts) {
            $productIndex = 0;
        }

        $sortInfo = SortHelper::parseSortType();
        $data = $this->modelProducts->rangeOfProducts($productIndex, self::PRODUCTS_PER_PAGE, $sortInfo["key"], $sortInfo["direction"]);
